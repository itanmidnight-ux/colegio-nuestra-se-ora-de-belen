...
$secciones = [];
$res = $conn->query("SELECT id, titulo, descripcion, contenido, imagen, contenido_extra, orden_visual FROM secciones_periodico ORDER BY orden_visual ASC, creado_en DESC");
if ($res) {
        ...
        <div class="list-item" style="margin-bottom:12px;">
          ...
          <div class="action-buttons">
            <button class="btn-edit btn-open-edit"
              ...
              data-extra='<?= htmlspecialchars($s['contenido_extra'] ?? '[]', ENT_QUOTES) ?>'
...
<div id="modalAdd" class="modal"><div class="modal-content">
  ...
  <form id="formAdd">
    ...
    <input type="hidden" name="contenido_extra" id="add_contenido_extra" value="[]">
    ...
    <label>Imagen principal de la sección</label><input type="file" id="add_file" accept="image/jpeg,image/png,image/webp">

    <div style="margin-top:12px; border-top:1px solid #ddd; padding-top:10px;">
      <h4>Contenido adicional (texto, imagen, video)</h4>
      <button type="button" class="btn-view" id="addExtraBlockBtn" style="margin-bottom:10px;">➕ Agregar más información</button>
      <div id="extraBlocksContainer"></div>
    </div>
...
<div id="modalEdit" class="modal"><div class="modal-content">
  ...
  <form id="formEdit">
    ...
    <input type="hidden" name="contenido_extra" id="edit_contenido_extra" value="[]">
    ...
    <label>Reemplazar imagen principal</label><input type="file" id="edit_file" accept="image/jpeg,image/png,image/webp">

    <div style="margin-top:12px; border-top:1px solid #ddd; padding-top:10px;">
      <h4>Contenido adicional (texto, imagen, video)</h4>
      <button type="button" class="btn-view" id="editExtraBlockBtn" style="margin-bottom:10px;">➕ Agregar más información</button>
      <div id="editExtraBlocksContainer"></div>
    </div>
...
const addContainer = document.getElementById('extraBlocksContainer');
const editContainer = document.getElementById('editExtraBlocksContainer');

function blockTemplate(data = {}) {
  const tipo = data.tipo || 'texto';
  const valor = data.valor || '';
  const orden = Number.isInteger(data.orden) ? data.orden : 0;
  return `
    <div class="extra-block" style="border:1px solid #dce7f8; border-radius:10px; padding:10px; margin-bottom:10px;">
      <label>Tipo</label>
      <select class="extra-tipo">
        ...
        <option value="video" ${tipo === 'video' ? 'selected' : ''}>Video (URL YouTube/MP4)</option>
      </select>
      <label>Contenido / URL</label>
      <textarea class="extra-valor" placeholder="Escribe texto o pega URL de imagen/video">${valor}</textarea>
      <label>Orden</label>
      <input type="number" class="extra-orden" value="${orden}">
      <button type="button" class="btn-delete remove-extra" style="margin-top:8px;">Quitar bloque</button>
    </div>`;
}

function bindRemove(container) {
  container.querySelectorAll('.remove-extra').forEach(btn => {
    btn.onclick = () => btn.closest('.extra-block').remove();
  });
}

document.getElementById('addExtraBlockBtn').addEventListener('click', () => {
  addContainer.insertAdjacentHTML('beforeend', blockTemplate());
  bindRemove(addContainer);
});
document.getElementById('editExtraBlockBtn').addEventListener('click', () => {
  editContainer.insertAdjacentHTML('beforeend', blockTemplate());
  bindRemove(editContainer);
});

function getBlocksData(container) {
  const data = [];
  container.querySelectorAll('.extra-block').forEach(block => {
    const tipo = block.querySelector('.extra-tipo').value;
    const valor = block.querySelector('.extra-valor').value.trim();
    const orden = parseInt(block.querySelector('.extra-orden').value || '0', 10);
    if (valor) data.push({ tipo, valor, orden: isNaN(orden) ? 0 : orden });
  });
...
document.querySelectorAll('.btn-open-edit').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  extra.sort((a,b) => (a.orden || 0) - (b.orden || 0));
  extra.forEach(item => editContainer.insertAdjacentHTML('beforeend', blockTemplate(item)));
  bindRemove(editContainer);
...
async function sendForm(form, action, modal){
  ...
  if (form.id === 'formAdd') {
    fd.set('contenido_extra', JSON.stringify(getBlocksData(addContainer)));
  }
  if (form.id === 'formEdit') {
    fd.set('contenido_extra', JSON.stringify(getBlocksData(editContainer)));
  }
